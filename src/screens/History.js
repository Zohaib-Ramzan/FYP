...
const History = () => {
  ...
  useEffect(() => {
    let unsubscribe; // Initialize unsubscribe variable
    ...
    const fetchPredictions = () => {
      ...
      const q = query(
        collection(FIRESTORE_DB, 'predictions'),
        where('uid', '==', users.uid),
    ...
    return () => {
      if (unsubscribe) {
        unsubscribe(); // Only call unsubscribe if it exists
      }
    };
  }, [users.uid]);
    ...
    // Delete History Blocks
    const handleDelete = async (id) => {
      try {
        await deleteDoc(doc(FIRESTORE_DB, 'predictions', id));
    ...
    <SafeAreaView style={styles.container}>
      <StatusBar backgroundColor="#6200EE" barStyle="dark-content" />
        <View style={styles.headingContainer}>
          <CustomText style={styles.headingStyle}>{`Welcome back, ${loading ? 'Loading...' : users.name}!`}</CustomText>
